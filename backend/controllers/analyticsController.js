...
export async function getAnalytics(req, res, next) {
  try {
    ...
    if (!allResumes.length) return res.json({ empty: true });
    ...
    // Average score
    const scores = allResumes.map(r => r.screening?.score).filter(s => typeof s === 'number');
    ...
    allResumes.forEach(r => {
      const rec = r.screening?.recommendation || 'Unknown';
      recMap[rec] = (recMap[rec] || 0) + 1;
    });
    const recommendations = Object.entries(recMap).map(([name, value]) => ({ name, value }));
    ...
    const sessionScores = sessions.map(s => ({
      ...
      avgScore: s.resumes.length
        ? Math.round(s.resumes.reduce((a, r) => a + (r.screening?.score || 0), 0) / s.resumes.length)
        : 0,
      count: s.resumes.length
    })).filter(s => s.count > 0);

    res.json({ totalResumes, totalSessions, avgScore, scoreDist, recommendations, topSkills, sessionScores });
  } catch (err) { next(err); }
}
